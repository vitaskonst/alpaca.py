...
void replace(std::string &str, const std::string &needle,
...
std::vector<gpt_vocab::id> llama_tokenize(const gpt_vocab &vocab,
                                          const std::string &text, bool bos) {
...
void sample_top_k(std::vector<std::pair<double, gpt_vocab::id>> &logits_id,
                  int top_k) {
...
gpt_vocab::id
llama_sample_top_p_top_k(const gpt_vocab &vocab, const float *logits,
...

  {
    ...
    for (int i = 0; i < n_logits; ++i) {
      ...
      if (std::find(last_n_tokens.begin(), last_n_tokens.end(), i) !=
          last_n_tokens.end()) {
...
size_t ggml_quantize_q4_0(float *src, void *dst, int n, int k, int qk,
  ...
  for (int j = 0; j < n; j += k) {
    ...
    uint8_t *pb =
        (uint8_t *)(pdst + (j / k) * row_size + 0 * bs + sizeof(float));
