...
bool llama_model_load(const std::string &fname, llama_model &model,
                      gpt_vocab &vocab, int n_ctx, bool verbose) {
  if (verbose)
  ...
  // verify magic
  {
    ...
    if (magic != 0x67676d6c) {
      fprintf(stderr, "ERROR: invalid model file '%s' (bad magic)\n",
              fname.c_str());
      return false;
    }
  }
  ...
  // load vocab
  {
    ...
    if (n_vocab != model.hparams.n_vocab) {
      fprintf(stderr,
              "ERROR: invalid model file '%s' (bad vocab size %d != %d)\n",
              fname.c_str(), n_vocab, model.hparams.n_vocab);
      return false;
    }
  ...
  case 3:
    ...
    break;
  default: {
    fprintf(stderr, "ERROR: invalid model file '%s' (bad f16 value %d)\n",
        ...
        if (n_dims == 1 || n_parts == 1) {
          ...
          if (split_type == 0) {
            ...
            for (int i1 = 0; i1 < ne[1]; ++i1) {
              ...
              const size_t offset =
                  ...
                  ((part_id * np0) / ggml_blck_size(tensor->type)) *
                      ggml_type_size(tensor->type);
              fin.read(reinterpret_cast<char *>(tensor->data) + offset,
                       row_size / n_parts);
            }
          } else {
            const int np1 = ne[1];
            ...
            for (int i1 = 0; i1 < ne[1]; ++i1) {
              ...
              fin.read(reinterpret_cast<char *>(tensor->data) + offset_row,
                       row_size);
            }
          }
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // feed-forward network
    {
      // norm
      {
        ...
        // cur = ffn_norm*cur
        cur = ggml_mul(ctx0, ggml_repeat(ctx0, model.layers[il].ffn_norm, cur),
                       cur);
      }
...
bool llama_eval_t(const llama_model &model, const int n_threads,
                  const int n_past, const std::vector<gpt_vocab::id> &embd_inp,
                  std::vector<float> &embd_w, size_t &mem_per_token,
                  int64_t &duration_us) {
  const int64_t t_start_us = ggml_time_us();
  bool success =
      llama_eval(model, n_threads, n_past, embd_inp, embd_w, mem_per_token);
  const int64_t t_end_us = ggml_time_us();
  duration_us = t_end_us - t_start_us;
  return success;
}
