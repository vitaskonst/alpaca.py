...
// GGML_FP32_TO_FP16 elsewhere for NEON.
...
float ggml_fp16_to_fp32(ggml_fp16_t x) { return GGML_FP16_TO_FP32(x); }
...
// represented with a single float (delta) and QK/2 8-bit ints (i.e QK 4-bit
// signed integer factors)
void quantize_row_q4_0(const float *restrict x, void *restrict y, int k) {
  ...
  for (int i = 0; i < nb; i++) {
    ...
    amax = MAX(MAX(vgetq_lane_f32(amaxv[0], 0), vgetq_lane_f32(amaxv[0], 1)),
...
#error "not implemented for QK"
  ...
  for (int i = 0; i < nb; i++) {
    ...
    for (int l = 0; l < 8; l++)
      srcv[l] = wasm_v128_load(x + i * 32 + 4 * l);
    for (int l = 0; l < 8; l++)
      asrcv[l] = wasm_f32x4_abs(srcv[l]);

    for (int l = 0; l < 4; l++)
      amaxv[2 * l] = wasm_f32x4_max(asrcv[2 * l], asrcv[2 * l + 1]);
    for (int l = 0; l < 2; l++)
      amaxv[4 * l] = wasm_f32x4_max(amaxv[4 * l], amaxv[4 * l + 2]);
    for (int l = 0; l < 1; l++)
      amaxv[8 * l] = wasm_f32x4_max(amaxv[8 * l], amaxv[8 * l + 4]);

    amax = MAX(MAX(wasm_f32x4_extract_lane(amaxv[0], 0),
    ...
    *(float *)pd = d;
    ...
    for (int l = 0; l < 8; l++) {
      ...
      pp[2 * l + 0] = wasm_i32x4_extract_lane(vi, 0) |
                      (wasm_i32x4_extract_lane(vi, 1) << 4);
      pp[2 * l + 1] = wasm_i32x4_extract_lane(vi, 2) |
                      (wasm_i32x4_extract_lane(vi, 3) << 4);
    }
...
// method 4
// blocks of QK elements
// represented with 2 floats (min + delta) and QK/2 8-bit ints (i.e QK 4-bit
// unsigned integer factors)
...
#define GGML_F32x4_REDUCE_ONE(x)                                               \
  (vgetq_lane_f32(x, 0) + vgetq_lane_f32(x, 1) + vgetq_lane_f32(x, 2) +        \
   vgetq_lane_f32(x, 3))
#endif
...
inline static void ggml_vec_dot_f32(const int n, float *restrict s,
...
inline static void ggml_vec_dot_f16(const int n, float *restrict s,
                                    ggml_fp16_t *restrict x,
...
inline static void ggml_vec_dot_q4_0(const int n, float *restrict s,
...
#else
    sum0 += d0_0 * d1_0 *
            (vgetq_lane_s32(p_0, 0) + vgetq_lane_s32(p_0, 1) +
             vgetq_lane_s32(p_0, 2) + vgetq_lane_s32(p_0, 3));
    sum1 += d0_1 * d1_1 *
            (vgetq_lane_s32(p_1, 0) + vgetq_lane_s32(p_1, 1) +
             vgetq_lane_s32(p_1, 2) + vgetq_lane_s32(p_1, 3));
#endif
#else
...
#else
    sum0 += d0_0 * d1_0 *
            (vgetq_lane_s16(p_0, 0) + vgetq_lane_s16(p_0, 1) +
             vgetq_lane_s16(p_0, 2) + vgetq_lane_s16(p_0, 3) +
             vgetq_lane_s16(p_0, 4) + vgetq_lane_s16(p_0, 5) +
             vgetq_lane_s16(p_0, 6) + vgetq_lane_s16(p_0, 7));
    sum1 += d0_1 * d1_1 *
            (vgetq_lane_s16(p_1, 0) + vgetq_lane_s16(p_1, 1) +
             vgetq_lane_s16(p_1, 2) + vgetq_lane_s16(p_1, 3) +
             vgetq_lane_s16(p_1, 4) + vgetq_lane_s16(p_1, 5) +
             vgetq_lane_s16(p_1, 6) + vgetq_lane_s16(p_1, 7));
#endif
#endif
  }
...
#error "not implemented for QK"
  ...
  for (int i = 0; i < nb; ++i) {
    ...
    const __m256 scale =
        _mm256_mul_ps(_mm256_broadcast_ss(d0_0), _mm256_broadcast_ss(d1_0));
...
#error "not implemented for QK"
  ...
  for (int i = 0; i < nb; i += 2) {
    ...
    // dot product into int16x8_t
    const v128_t pl0l = wasm_i16x8_mul(wasm_i16x8_extend_low_i8x16(v0_0ls),
                                       wasm_i16x8_extend_low_i8x16(v1_0ls));
    const v128_t pl0h = wasm_i16x8_mul(wasm_i16x8_extend_high_i8x16(v0_0ls),
                                       wasm_i16x8_extend_high_i8x16(v1_0ls));

    const v128_t ph0l = wasm_i16x8_mul(wasm_i16x8_extend_low_i8x16(v0_0hs),
                                       wasm_i16x8_extend_low_i8x16(v1_0hs));
    const v128_t ph0h = wasm_i16x8_mul(wasm_i16x8_extend_high_i8x16(v0_0hs),
                                       wasm_i16x8_extend_high_i8x16(v1_0hs));

    const v128_t pl1l = wasm_i16x8_mul(wasm_i16x8_extend_low_i8x16(v0_1ls),
                                       wasm_i16x8_extend_low_i8x16(v1_1ls));
    const v128_t pl1h = wasm_i16x8_mul(wasm_i16x8_extend_high_i8x16(v0_1ls),
                                       wasm_i16x8_extend_high_i8x16(v1_1ls));

    const v128_t ph1l = wasm_i16x8_mul(wasm_i16x8_extend_low_i8x16(v0_1hs),
                                       wasm_i16x8_extend_low_i8x16(v1_1hs));
    const v128_t ph1h = wasm_i16x8_mul(wasm_i16x8_extend_high_i8x16(v0_1hs),
                                       wasm_i16x8_extend_high_i8x16(v1_1hs));
...
inline static void ggml_vec_dot_q4_1(const int n, float *restrict s,
                                     const void *restrict x,
                                     const void *restrict y) {
  const int nb = n / QK;
...
inline static void ggml_vec_mad_f16(const int n, ggml_fp16_t *restrict y,
                                    ggml_fp16_t *restrict x, const float v) {
#if defined(GGML_SIMD)
  const int np = (n & ~(GGML_F16_STEP - 1));
...
inline static void ggml_vec_mad_q4_0(const int n, float *restrict y,
                                     void *restrict x, const float v) {
  assert(n % QK == 0);
...
inline static void ggml_vec_mad_q4_1(const int n, float *restrict y,
                                     void *restrict x, const float v) {
  assert(n % QK == 0);
  ...
  for (int i = 0; i < nb; i++) {
    ...
    for (int l = 0; l < QK; l += 2) {
      ...
      assert(!isinf(y[i * QK + l + 0]));
      assert(!isinf(y[i * QK + l + 1]));
      // printf("mad: v0 %f v1 %f, i = %d, l = %d, d = %f, vi = %d, vi0 = %d,
...
inline static float ggml_gelu_f32(float x) {
  return 0.5 * x *
         (1.0 + tanh(SQRT_2_OVER_PI * x * (1.0 + GELU_COEF_A * x * x)));
}
...
inline static float ggml_silu_f32(float x) { return x / (1.0 + exp(-x)); }

inline static void ggml_vec_silu_f16(const int n, ggml_fp16_t *y,
                                     const ggml_fp16_t *x) {
  const uint16_t *i16 = (const uint16_t *)x;
  for (int i = 0; i < n; ++i) {
...
static const size_t GGML_TYPE_SIZE[GGML_TYPE_COUNT] = {
    sizeof(float) + QK / 2, sizeof(float) * 2 + QK / 2,
    sizeof(int8_t),         sizeof(int16_t),
    sizeof(int32_t),        sizeof(ggml_fp16_t),
    sizeof(float),
};
...
static const char *GGML_OP_LABEL[GGML_OP_COUNT] = {
    ...
    "DUP",        "ADD",        "SUB",           "MUL",      "DIV",
    "SQR",        "SQRT",       "SUM",           "MEAN",     "REPEAT",
    "ABS",        "SGN",        "NEG",           "STEP",     "RELU",
    "GELU",       "SILU",       "NORM",          "RMS_NORM",

    "MUL_MAT",

    "SCALE",      "CPY",        "RESHAPE",       "VIEW",     "PERMUTE",
    "TRANSPOSE",  "GET_ROWS",   "DIAG_MASK_INF", "SOFT_MAX", "ROPE",
    "CONV_1D_1S", "CONV_1D_2S",
...
static inline bool ggml_can_mul_mat(const struct ggml_tensor *t0,
  ...
  return (t0->ne[0] == t1->ne[0]) && (t0->ne[2] == t1->ne[2]) &&
         (t0->ne[3] == t1->ne[3]);
}
...
static inline bool ggml_are_same_shape(const struct ggml_tensor *t0,
                                       const struct ggml_tensor *t1) {
  static_assert(GGML_MAX_DIMS == 4,
                "GGML_MAX_DIMS is not 4 - update this function");
...
static inline int ggml_up64(int n) { return (n + 63) & ~63; }
...
struct ggml_tensor *ggml_new_tensor_impl(struct ggml_context *ctx,
                                         enum ggml_type type, int n_dims,
                                         const int *ne, void *data) {
  ...
  if (data == NULL) {
    ...
    size_needed =
        ((size_needed + GGML_MEM_ALIGN - 1) / GGML_MEM_ALIGN) * GGML_MEM_ALIGN;
  }
  ...
  if (ctx->scratch.data == NULL || data != NULL) {
    ...
    if (cur_end + size_needed + GGML_OBJECT_SIZE > ctx->mem_size) {
      GGML_PRINT("%s: not enough space in the context's memory pool (needed "
                 "%zu, available %zu)\n",
                 __func__, cur_end + size_needed + GGML_OBJECT_SIZE,
                 ctx->mem_size);
      assert(false);
      return NULL;
    }
    ...
    *obj_new = (struct ggml_object){
    ...
    if (cur_end + sizeof(struct ggml_tensor) + GGML_OBJECT_SIZE >
        ctx->mem_size) {
      GGML_PRINT("%s: not enough space in the context's memory pool (needed "
                 "%zu, available %zu)\n",
                 __func__,
                 cur_end + sizeof(struct ggml_tensor) + GGML_OBJECT_SIZE,
                 ctx->mem_size);
      assert(false);
      return NULL;
    }
  ...
  struct ggml_tensor *const result =
      (struct ggml_tensor *)(mem_buffer + obj_new->offs);
...
struct ggml_tensor *ggml_dup_impl(struct ggml_context *ctx,
...
struct ggml_tensor *ggml_add_impl(struct ggml_context *ctx,
...
struct ggml_tensor *ggml_sub_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, struct ggml_tensor *b,
                                  bool inplace) {
  GGML_ASSERT(ggml_are_same_shape(a, b));
...
struct ggml_tensor *ggml_mul_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, struct ggml_tensor *b,
                                  bool inplace) {
  GGML_ASSERT(ggml_are_same_shape(a, b));
...
struct ggml_tensor *ggml_div_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, struct ggml_tensor *b,
                                  bool inplace) {
  GGML_ASSERT(ggml_are_same_shape(a, b));
...
struct ggml_tensor *ggml_sqr_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_sqrt_impl(struct ggml_context *ctx,
                                   struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_abs_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_sgn_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_neg_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_step_impl(struct ggml_context *ctx,
                                   struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_relu_impl(struct ggml_context *ctx,
                                   struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_gelu_impl(struct ggml_context *ctx,
                                   struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_silu_impl(struct ggml_context *ctx,
                                   struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_norm_impl(struct ggml_context *ctx,
                                   struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_rms_norm_impl(struct ggml_context *ctx,
                                       struct ggml_tensor *a, bool inplace) {
  bool is_node = false;
...
struct ggml_tensor *ggml_scale_impl(struct ggml_context *ctx,
                                    struct ggml_tensor *a,
...
struct ggml_tensor *ggml_cpy_impl(struct ggml_context *ctx,
                                  struct ggml_tensor *a, struct ggml_tensor *b,
                                  bool inplace) {
...
struct ggml_tensor *ggml_reshape(struct ggml_context *ctx,
  ...
  struct ggml_tensor *result =
      ggml_new_tensor_impl(ctx, a->type, b->n_dims, b->ne, a->data);
...
struct ggml_tensor *ggml_reshape_2d(struct ggml_context *ctx,
  ...
  const int ne[2] = {ne0, ne1};
  struct ggml_tensor *result =
      ggml_new_tensor_impl(ctx, a->type, 2, ne, a->data);
...
struct ggml_tensor *ggml_reshape_3d(struct ggml_context *ctx,
  ...
  const int ne[3] = {ne0, ne1, ne2};
  struct ggml_tensor *result =
      ggml_new_tensor_impl(ctx, a->type, 3, ne, a->data);
...
struct ggml_tensor *ggml_view_1d(struct ggml_context *ctx,
  ...
  struct ggml_tensor *result =
      ggml_new_tensor_impl(ctx, a->type, 1, &ne0, (char *)a->data + offset);
...
struct ggml_tensor *ggml_view_2d(struct ggml_context *ctx,
  ...
  struct ggml_tensor *result =
      ggml_new_tensor_impl(ctx, a->type, 2, ne, (char *)a->data + offset);
...
ggml_compute_forward_dup_f16(const struct ggml_compute_params *params,
  ...
  if (ggml_is_contiguous(src0) && src0->type == dst->type) {
    memcpy(dst->data, src0->data,
           ggml_nelements(dst) * GGML_TYPE_SIZE[src0->type]);
    return;
  }
...
static void ggml_compute_forward_add_f32(
  ...
  if (nb10 == sizeof(float)) {
    ...
    for (int j = j0; j < j1; j++) {
      ggml_vec_add_f32(nc, (float *)((char *)dst->data + j * nb1),
...
static void ggml_compute_forward_sub_f32(
  ...
  for (int i = 0; i < n; i++) {
    ggml_vec_sub_f32(nc, (float *)((char *)dst->data + i * (dst->nb[1])),
...
static void ggml_compute_forward_mul_f32(
  ...
  for (int i = 0; i < n; i++) {
    ggml_vec_mul_f32(nc, (float *)((char *)dst->data + i * (dst->nb[1])),
                     (float *)((char *)src0->data + i * (src0->nb[1])),
                     (float *)((char *)src1->data + i * (src1->nb[1])));
  }
}
...
static void ggml_compute_forward_div_f32(
  ...
  for (int i = 0; i < n; i++) {
    ggml_vec_div_f32(nc, (float *)((char *)dst->data + i * (dst->nb[1])),
                     (float *)((char *)src0->data + i * (src0->nb[1])),
                     (float *)((char *)src1->data + i * (src1->nb[1])));
  }
}
...
ggml_compute_forward_sum_f32(const struct ggml_compute_params *params,
  ...
  for (int i03 = 0; i03 < ne03; i03++) {
    for (int i02 = 0; i02 < ne02; i02++) {
      for (int i01 = 0; i01 < ne01; i01++) {
        ggml_vec_sum_f32(ne00, (float *)(dst->data),
    ...
    for (int i02 = 0; i02 < ne02; i02++) {
      for (int i01 = 0; i01 < ne01; i01++) {
        ggml_vec_sum_f32(
            ...
            (float *)((char *)src0->data + i01 * nb01 + i02 * nb02 +
                      i03 * nb03));
...
ggml_compute_forward_repeat_f32(const struct ggml_compute_params *params,
  ...
  const int ncr =
      nc /
      nc0; // guaranteed to be an integer due to the check in ggml_can_repeat
  const int nrr =
      nr /
      nr0; // guaranteed to be an integer due to the check in ggml_can_repeat
...
static void ggml_compute_forward_mul_mat_f32(
  ...
  if (ggml_compute_forward_mul_mat_use_blas(src0, src1, dst)) {
    ...
    for (int i03 = 0; i03 < ne03; i03++) {
      for (int i02 = 0; i02 < ne02; i02++) {
        ...
        const float *y =
            (float *)((char *)src1->data + i02 * nb12 + i03 * nb13);
        ...
        // zT = y * xT
        {
          cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, ne11, ne01, ne10,
                      1.0f, y, ne10, x, ne10, 0.0f, d, ne01);
        }
      }
    }
  ...
  if (params->type == GGML_TASK_FINALIZE) {
    ...
    for (int k = 1; k < nth; k++) {
      ggml_vec_acc_f32(ic1 - ic0, (float *)dst->data + ic0,
                       wdata + (ne + CACHE_LINE_SIZE_F32) * k + ic0);
    }
      ...
      for (int i12 = 0; i12 < ne12; ++i12) {
        for (int i11 = 0; i11 < ne11; ++i11) {
          for (int ic = ic0; ic < ic1; ++ic) {
            ...
            assert(sizeof(float) * (wo + i3 * ne2 * ne1 * ne0 + i2 * ne1 * ne0 +
                                    i1 * ne0 + ne01) <=
                   params->wsize);

            ggml_vec_mad_f32(
                ne01,
                (float *)(wdata + wo + i3 * ne2 * ne1 * ne0 + i2 * ne1 * ne0 +
                          i1 * ne0),
                (float *)((char *)src0->data +
                          (i00 * nb00 + i02 * nb02 + i03 * nb03)),
                *(float *)((char *)src1->data + (i10 * nb10 + i11 * nb11 +
                                                 i12 * nb12 + i13 * nb13)));
          }
        }
      }
  ...
  //     ne13 = %5d\n", ne10, ne11, ne12, ne13); printf("nb10 = %5d, nb11 = %5d,
  //     nb12 = %5d, nb13 = %5d\n", nb10, nb11, nb12, nb13);
...
static void ggml_compute_forward_mul_mat_f16_f32(
      ...
      for (int i02 = 0; i02 < ne02; i02++) {
        {
          ...
          for (int i01 = 0; i01 < ne01; ++i01) {
            for (int i00 = 0; i00 < ne00; ++i00) {
              wdata[id++] = GGML_FP16_TO_FP32(
                  *(ggml_fp16_t *)((char *)src0->data + i03 * nb03 +
                                   i02 * nb02 + i01 * nb01 + i00 * nb00));
            }
          }
        }
...

        {
          ...
          // zT = y * xT
          cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, ne11, ne01, ne10,
                      1.0f, y, ne00, x, ne00, 0.0f, d, ne01);
#else
          // zT = (xT * y)T
          cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, ne01, ne11, ne10,
                      1.0f, x, ne00, y, ne00, 0.0f, d, ne01);
#endif
        }
      }
    }
  ...
  if (params->type == GGML_TASK_FINALIZE) {
    ...
    for (int k = 1; k < nth; k++) {
      for (int i = ic0; i < ic1; ++i) {
        ((float *)dst->data)[i] +=
            GGML_FP16_TO_FP32(wdata[(ne + CACHE_LINE_SIZE_F32) * k + i]);
      }
    }
  ...
  if (nb01 >= nb00) {
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
      ...
      ggml_fp16_t *src0_row =
          (ggml_fp16_t *)((char *)src0->data +
                          (i01 * nb01 + i02 * nb02 + i03 * nb03));
      ggml_fp16_t *src1_col =
          wdata + (0 + i12 * ne11 + i13 * ne12 * ne11) * ne00;
      ...
      for (int ic = 0; ic < ne11; ++ic) {
        ggml_vec_dot_f16(ne00, &dst_col[ic * ne0], src0_row,
                         src1_col + ic * ne00);
      }
    }
  } else {
      ...
      for (int i12 = 0; i12 < ne12; ++i12) {
        for (int i11 = 0; i11 < ne11; ++i11) {
          ...
          for (int ic = ic0; ic < ic1; ++ic) {
            ...
            assert(sizeof(ggml_fp16_t) * (wo + i3 * ne2 * ne1 * ne0 +
                                          i2 * ne1 * ne0 + i1 * ne0 + ne01) <=
                   params->wsize);

            ggml_fp16_t *src0_col =
                (ggml_fp16_t *)((char *)src0->data +
                                (i00 * nb00 + i02 * nb02 + i03 * nb03));
            float src1_val =
                *(float *)((char *)src1->data +
...
static void ggml_compute_forward_mul_mat_q4_0_f32(
  ...
  GGML_ASSERT(nb00 == (int)GGML_TYPE_SIZE[GGML_TYPE_Q4_0] ||
              nb01 == (int)GGML_TYPE_SIZE[GGML_TYPE_Q4_0]);
      ...
      for (int i02 = 0; i02 < ne02; i02++) {
        {
          ...
          for (int i01 = 0; i01 < ne01; ++i01) {
            // for (int i00 = 0; i00 < ne00; ++i00) {
            //     wdata[id++] = GGML_FP16_TO_FP32(*(ggml_fp16_t *) ((char *)
            //     src0->data + i03*nb03 + i02*nb02 + i01*nb01 + i00*nb00));
            // }
            dequantize_row_q4_0((char *)src0->data + i03 * nb03 + i02 * nb02 +
                                    i01 * nb01,
                                wdata + id, ne00);
            id += ne00;
          }
    ...
    if (nb01 >= nb00) {
      ...
      for (int i13 = 0; i13 < ne13; ++i13) {
        for (int i12 = 0; i12 < ne12; ++i12) {
          for (int i11 = 0; i11 < ne11; ++i11) {
            // for (int i10 = 0; i10 < ne10; ++i10) {
            //     wdata[id++] = GGML_FP32_TO_FP16(*(float *)((char *)
            //     src1->data + i13*nb13 + i12*nb12 + i11*nb11 + i10*nb10));
            // }
            quantize_row_q4_0((float *)((char *)src1->data + i13 * nb13 +
                                        i12 * nb12 + i11 * nb11),
                              (void *)wdata, ne10);
            wdata += (ne10 * GGML_TYPE_SIZE[GGML_TYPE_Q4_0]) /
                     GGML_BLCK_SIZE[GGML_TYPE_Q4_0];
          }
        }
      }
  ...
  if (nb01 >= nb00) {
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
      ...
      char *src1_col =
          ((char *)wdata + ((0 + i12 * ne11 + i13 * ne12 * ne11) * ne00 *
      ...
      for (int ic = 0; ic < ne11; ++ic) {
        ggml_vec_dot_q4_0(
            ne00, &dst_col[ic * ne0], src0_row,
            ((void *)(src1_col + (ic * ne00 * GGML_TYPE_SIZE[GGML_TYPE_Q4_0]) /
                                     GGML_BLCK_SIZE[GGML_TYPE_Q4_0])));
      }
    }
  } else {
      ...
      for (int i12 = 0; i12 < ne12; ++i12) {
        for (int i11 = 0; i11 < ne11; ++i11) {
          ...
          for (int ic = ic0; ic < ic1; ++ic) {
            ...
            void *src0_col = (void *)((char *)src0->data +
                                      (i00 * nb00 + i02 * nb02 + i03 * nb03));
            float src1_val =
                *(float *)((char *)src1->data +
                           (i10 * nb10 + i11 * nb11 + i12 * nb12 + i13 * nb13));
...
static void ggml_compute_forward_mul_mat_q4_1_f32(
  ...
  GGML_ASSERT(nb00 == (int)GGML_TYPE_SIZE[GGML_TYPE_Q4_1] ||
              nb01 == (int)GGML_TYPE_SIZE[GGML_TYPE_Q4_1]);
      ...
      for (int i02 = 0; i02 < ne02; i02++) {
        {
          ...
          for (int i01 = 0; i01 < ne01; ++i01) {
            ...
            //     src0->data + i03*nb03 + i02*nb02 + i01*nb01 + i00*nb00));
            // }
            dequantize_row_q4_1((char *)src0->data + i03 * nb03 + i02 * nb02 +
                                    i01 * nb01,
                                wdata + id, ne00);
            id += ne00;
          }
      ...
      for (int i13 = 0; i13 < ne13; ++i13) {
        for (int i12 = 0; i12 < ne12; ++i12) {
          for (int i11 = 0; i11 < ne11; ++i11) {
            ...
            //     src1->data + i13*nb13 + i12*nb12 + i11*nb11 + i10*nb10));
            // }
            quantize_row_q4_1((float *)((char *)src1->data + i13 * nb13 +
                                        i12 * nb12 + i11 * nb11),
                              (void *)wdata, ne10);
            wdata += (ne10 * GGML_TYPE_SIZE[GGML_TYPE_Q4_1]) /
                     GGML_BLCK_SIZE[GGML_TYPE_Q4_1];
          }
        }
      }
  ...
  if (nb01 >= nb00) {
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
      ...
      for (int ic = 0; ic < ne11; ++ic) {
        ggml_vec_dot_q4_1(
            ne00, &dst_col[ic * ne0], src0_row,
            ((void *)(src1_col + (ic * ne00 * GGML_TYPE_SIZE[GGML_TYPE_Q4_1]) /
                                     GGML_BLCK_SIZE[GGML_TYPE_Q4_1])));
      }
    }
  } else {
...
static void ggml_compute_forward_get_rows_q4_0(
  ...
  for (int i = 0; i < nr; ++i) {
    ...
    dequantize_row_q4_0((const void *)((char *)src0->data + r * src0->nb[1]),
...
static void ggml_compute_forward_get_rows_q4_1(
  ...
  for (int i = 0; i < nr; ++i) {
    ...
    dequantize_row_q4_1((const void *)((char *)src0->data + r * src0->nb[1]),
                        (float *)((char *)dst->data + i * dst->nb[1]), nc);
  }
}
...
static void ggml_compute_forward_get_rows(
  ...
  //         for (int j = 0; j < dst->ne[0]/16; ++j) {
  //             for (int i = 0; i < 16; ++i) {
  //                 printf("%8.4f ", ((float *) dst->data)[k*dst->ne[0] + j*16
  //                 + i]);
  //             }
  //             printf("\n");
  //         }
  ...
  for (int k = 0; k < nz; k++) {
    for (int j = 0; j < nr; j++) {
      for (int i = n_past; i < nc; i++) {
        if (i > n_past + j) {
          *(float *)((char *)dst->data + k * dst->nb[2] + j * dst->nb[1] +
                     i * dst->nb[0]) = -INFINITY;
        }
      }
    }
    ...
    for (int i0 = 0; i0 < ne10; ++i0) {
      ...
      for (int k = -nh; k <= nh; k++) {
        ...
        ggml_vec_dot_f16(ew0, &v,
                         (ggml_fp16_t *)params->wdata + i1 * ew0 * ne00 +
                             (nh + k) * ew0,
                         (ggml_fp16_t *)params->wdata + ne02 * ew0 * ne00 +
                             (i0 + nh + k) * ew0);
  ...
  for (int i1 = ir0; i1 < ir1; i1++) {
    ...
    for (int i0 = 0; i0 < ne10; ++i0) {
      ...
      for (int k = -nh; k <= nh; k++) {
        float v = 0.0f;
        ggml_vec_dot_f32(
            ew0, &v, (float *)params->wdata + i1 * ew0 * ne00 + (nh + k) * ew0,
...
ggml_compute_forward_flash_ff_f16(const struct ggml_compute_params *params,
  ...
  for (int ir = ir0; ir < ir1; ++ir) {
    ...
    ggml_fp16_t *S16 = (ggml_fp16_t *)((float *)params->wdata +
...
static void ggml_compute_forward(struct ggml_compute_params *params,
  ...
  case GGML_OP_DIAG_MASK_INF: {
    ggml_compute_forward_diag_mask_inf(params, tensor->src0, tensor->src1,
                                       tensor);
  } break;
  ...
  case GGML_OP_FLASH_ATTN: {
    ...
    GGML_ASSERT(t == 0 || t == 1);
    bool masked = t != 0;
    ggml_compute_forward_flash_attn(params, tensor->src0, tensor->src1,
                                    tensor->opt[0], masked, tensor);
  } break;
  case GGML_OP_FLASH_FF: {
    ggml_compute_forward_flash_ff(params, tensor->src0, tensor->src1,
                                  tensor->opt[0], tensor->opt[1],
                                  tensor->opt[2], tensor);
  } break;
  case GGML_OP_NONE: {
    // nop
...
static void ggml_compute_backward(struct ggml_context *ctx,
  ...
  case GGML_OP_MUL: {
    if (src0->grad) {
      src0->grad = ggml_add_impl(ctx, src0->grad,
                                 ggml_mul(ctx, src1, tensor->grad), inplace);
    }
    if (src1->grad) {
      src1->grad = ggml_add_impl(ctx, src1->grad,
                                 ggml_mul(ctx, src0, tensor->grad), inplace);
    }
  } break;
  case GGML_OP_DIV: {
    if (src0->grad) {
      src0->grad = ggml_add_impl(ctx, src0->grad,
                                 ggml_div(ctx, tensor->grad, src1), inplace);
    }
    if (src1->grad) {
      src1->grad = ggml_sub_impl(
          ctx, src1->grad,
          ggml_mul(ctx, tensor->grad, ggml_div(ctx, tensor, src1)), inplace);
    }
  } break;
  case GGML_OP_SQR: {
    if (src0->grad) {
      src0->grad = ggml_add_impl(
          ctx, src0->grad,
          ggml_mul(ctx, ggml_mul(ctx, src0, tensor->grad),
  ...
  case GGML_OP_SQRT: {
    if (src0->grad) {
      src0->grad = ggml_add_impl(
          ctx, src0->grad,
          ggml_div(ctx, ggml_repeat(ctx, ggml_new_f32(ctx, 0.5f), tensor),
                   tensor),
          inplace);
    }
  } break;
  case GGML_OP_SUM: {
    if (src0->grad) {
      src0->grad = ggml_add_impl(
          ctx, src0->grad, ggml_repeat(ctx, tensor->grad, src0->grad), inplace);
    }
  } break;
  case GGML_OP_MEAN: {
    GGML_ASSERT(false); // TODO: implement
  ...
  case GGML_OP_REPEAT: {
    if (src0->grad) {
      src0->grad =
          ggml_add_impl(ctx, src0->grad, ggml_sum(ctx, tensor->grad), inplace);
    }
  } break;
  case GGML_OP_ABS: {
    if (src0->grad) {
      src0->grad = ggml_add_impl(
          ctx, src0->grad, ggml_mul(ctx, ggml_sgn(ctx, src0), tensor->grad),
          inplace);
    }
  } break;
  case GGML_OP_SGN: {
  ...
  case GGML_OP_MUL_MAT: {
    if (src0->grad) {
      // TODO: this requires outer product - ggml_out_prod(ctx, src1,
      // tensor->grad);
      GGML_ASSERT(false);
    }
    if (src1->grad) {
      src1->grad = ggml_add_impl(
          ...
          ggml_mul_mat(ctx, ggml_transpose(ctx, src0), tensor->grad), inplace);
    }
  } break;
  case GGML_OP_SCALE: {
    GGML_ASSERT(false); // TODO: not implemented
...
static void ggml_visit_parents(struct ggml_cgraph *cgraph,
  ...
  if (node->grad == NULL) {
    // this usually happens when we generate intermediate nodes from constants
    // in the backward pass it can also happen during forward pass, if the user
    // performs computations with constants
...
struct ggml_cgraph ggml_build_backward(struct ggml_context *ctx,
  ...
  // if we are keeping the gradient graph, we have to detach the gradient nodes
  ...
  for (int i = gf->n_nodes - 1; i >= 0; i--) {
    ...
    // because we detached the grad nodes from the original graph, we can afford
    // inplace operations
    if (node->grad) {
...
void ggml_graph_compute(struct ggml_context *ctx, struct ggml_cgraph *cgraph) {
  ...
  struct ggml_compute_state *workers =
      n_threads > 1
          ? alloca(sizeof(struct ggml_compute_state) * (n_threads - 1))
          : NULL;
  ...
  if (n_threads > 1) {
    ...
    for (int j = 0; j < n_threads - 1; j++) {
      ...
      int rc = ggml_thread_create(&workers[j].thrd, NULL,
  ...
  // initialize tasks + work buffer
  {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->src0->nb[1] < node->src0->nb[0]) {
          ...
          if (node->src0->type == GGML_TYPE_F16 &&
...
#if defined(GGML_USE_ACCELERATE) || defined(GGML_USE_OPENBLAS)
            if (ggml_compute_forward_mul_mat_use_blas(node->src0, node->src1,
                                                      node)) {
              node->n_tasks = 1; // TODO: this actually is doing nothing
                                 //       the threads are still spinning
              cur = GGML_TYPE_SIZE[GGML_TYPE_F32] *
                    (node->src0->ne[0] * node->src0->ne[1]);
              // printf("src0: ne0 = %d, ne1 = %d, ne = %d\n",
              // node->src0->ne[0], node->src0->ne[1],
              // node->src0->ne[0]*node->src0->ne[1]); printf("src1: ne0 = %d,
              // ne1 = %d, ne = %d\n", node->src1->ne[0], node->src1->ne[1],
              // node->src1->ne[0]*node->src1->ne[1]); printf("cur = %zu\n",
              // cur);
            } else {
            ...
            if (ggml_compute_forward_mul_mat_use_blas(node->src0, node->src1,
              ...
              cur = (GGML_TYPE_SIZE[GGML_TYPE_Q4_0] *
                     ggml_nelements(node->src1)) /
                    GGML_BLCK_SIZE[GGML_TYPE_Q4_0];
            }
#else
            cur =
                (GGML_TYPE_SIZE[GGML_TYPE_Q4_0] * ggml_nelements(node->src1)) /
                GGML_BLCK_SIZE[GGML_TYPE_Q4_0];
#endif
            ...
            if (ggml_compute_forward_mul_mat_use_blas(node->src0, node->src1,
              ...
              cur = (GGML_TYPE_SIZE[GGML_TYPE_Q4_1] *
                     ggml_nelements(node->src1)) /
                    GGML_BLCK_SIZE[GGML_TYPE_Q4_1];
            }
#else
            cur =
                (GGML_TYPE_SIZE[GGML_TYPE_Q4_1] * ggml_nelements(node->src1)) /
                GGML_BLCK_SIZE[GGML_TYPE_Q4_1];
#endif
          } else {
            GGML_ASSERT(false);
      ...
      case GGML_OP_FLASH_FF: {
        ...
        if (node->src1->type == GGML_TYPE_F32) {
          cur = sizeof(float) * node->src1->ne[1] *
                node->n_tasks; // TODO: this can become (n_tasks-1)
          cur += sizeof(float) * node->src1->ne[1] *
                 node->n_tasks; // this is overestimated by x2
        }
  ...
  for (int i = 0; i < cgraph->n_nodes; i++) {
    ...
    // TODO: this could be used to avoid unnecessary computations, but it needs
    // to be improved
...
void ggml_graph_print(const struct ggml_cgraph *cgraph) {
  ...
  for (int i = 0; i < cgraph->n_nodes; i++) {
    ...
    GGML_PRINT(" - %3d: [ %6d, %6d, %6d] %16s %s (%3d) cpu = %7.3f / %7.3f ms, "
               "wall = %7.3f / %7.3f ms\n",
               i, node->ne[0], node->ne[1], node->ne[2],
...
label=\"%d [%d, %d] | <x>%s",
            (void *)node, color, i, node->ne[0], node->ne[1],
  ...
  for (int i = 0; i < gb->n_nodes; i++) {
    ...
    if (node->src0) {
      ...
      fprintf(fp,
              ...
              parent0 ? "g" : "x", parent ? (void *)parent : (void *)node,
              parent ? "g" : "x", parent ? "empty" : "vee",
              parent ? "dashed" : "solid");
    }
    ...
    if (node->src1) {
      ...
      fprintf(fp,
              ...
              parent1 ? "g" : "x", parent ? (void *)parent : (void *)node,
              parent ? "g" : "x", parent ? "empty" : "vee",
              parent ? "dashed" : "solid");
    }
  }
...
static enum ggml_opt_result ggml_opt_adam(struct ggml_context *ctx,
  ...
  float *x = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)
                 ->data; // view of the parameters
  float *g1 = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // gradient
  float *g2 =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // gradient squared
  ...
  float *mh =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // first moment hat
  float *vh =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // second moment hat

  float *pf = params.past > 0
                  ? ggml_new_tensor_1d(ctx, GGML_TYPE_F32, params.past)->data
...
static enum ggml_opt_result linesearch_backtracking(
    struct ggml_context *ctx, const struct ggml_opt_params *params, int nx,
    ...
    const int np, struct ggml_tensor *ps[]) {
  int count = 0;
...
static enum ggml_opt_result ggml_opt_lbfgs(struct ggml_context *ctx,
                                           struct ggml_opt_params params,
                                           struct ggml_tensor *f,
                                           struct ggml_cgraph *gf,
                                           struct ggml_cgraph *gb) {
  ...
  float *x =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // current parameters
  float *xp =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // previous parameters
  float *g =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // current gradient
  float *gp =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // previous gradient
  float *d =
      ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data; // search direction
  ...
  // the L-BFGS memory
  struct ggml_lbfgs_iteration_data *lm =
      alloca(sizeof(struct ggml_lbfgs_iteration_data) * m);
  ...
  while (true) {
    ...
    ls = linesearch_backtracking(ctx, &params, nx, x, &fx, g, d, &step, xp, f,
                                 gf, gb, np, ps);
