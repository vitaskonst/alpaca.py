...
// quantize a model
bool llama_model_quantize(const std::string &fname_inp,
                          const std::string &fname_out, int itype) {
  ...
  // load weights
  {
    ...
    while (true) {
...

      {
        ...
        printf("%48s - [%5d, %5d], type = %6s ", name.data(), ne[0], ne[1],
               ftype_str[ftype]);
      }
...
// usage:
//  ./llama-quantize models/llama/ggml-model.bin
//  models/llama/ggml-model-quant.bin type
//
